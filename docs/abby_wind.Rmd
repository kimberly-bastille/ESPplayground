...
## Read in wind data
```{r, echo = TRUE}
vwind <- raster::stack(here::here("data-raw/vwnd.10m.2021.nc"))
```
```{r, echo = TRUE}
#vwind <- raster::crop(vwind, extent(280,300,30,50)) 
# does not run in Rmarkdown???
```
```{r, echo = TRUE}
vwind <- raster::rotate(vwind)
```
```{r, echo = TRUE}
raster::crs(vwind) <- "+proj=longlat +lat_1=35 +lat_2=45 +lat_0=40 +lon_0=-77 +x_0=0 +y_0=0 +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0"
```

## Mask to relevant strata
```{r, echo = TRUE}
## not sure why this mask isn't working??
m_vwind <- raster::mask(vwind, new_shape[new_shape$STRATA > 2000 & new_shape$STRATA < 8000,2])
mean(m_vwind@data@values, na.rm = TRUE)
m_vwind[[1]]
#
# uwind <- here::here("data-raw/uwnd.10m.2021.nc") %>%
#   raster::stack()

# something is wrong here
vwind[[1]]

# vwind_mask <- raster::mask(vwind, new_shape[new_shape$STRATA > 2000 & new_shape$STRATA < 8000,])
# vwind_mean <- mean(vwind_mask@data@values, na.rm = TRUE)
