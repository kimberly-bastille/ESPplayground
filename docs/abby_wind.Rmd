...
dat <- dplyr::full_join(recruit, all_wind) %>%
  tidyr::drop_na(Recruitment, crossshore_wind, longshore_wind) %>%
  dplyr::select(Year, region, Recruitment, month, longshore_wind, crossshore_wind) %>%
  dplyr::filter(month == 4 | month == 5) %>%
  dplyr::mutate(month = ifelse(month == 4, "April", "May")) %>%
  tidyr::pivot_wider(names_from = c(month, region),
                     values_from = c(crossshore_wind, longshore_wind)) %>%
  tidyr::drop_na()

eqn <- as.formula(paste("Recruitment ~", paste(colnames(dat)[3:10], collapse = " + ")))
...
MASS::stepAIC(mod0, scope = list(upper = ~ crossshore_wind_April_North + crossshore_wind_May_North + 
...
wind_indicators <- dat %>%
  ...
  tidyr::pivot_longer(cols = colnames(dat)[3:10])

write.csv(wind_indicators, "wind_indicators.csv")
```
